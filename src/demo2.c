...
void demo(char *cfgfile, char *weightfile, float thresh, int cam_index, const char *filename, char **names, int classes, int delay, char *prefix, int avg_frames, float hier, int w, int h, int frames, int fullscreen, const char *outputvideo)
{
    ...
    while(!demo_done){
        ...
        if(!prefix){
            ...
            if(outputvideo){
                ...
                if(writer == NULL){
                    ...
                    writer = cvCreateVideoWriter(outputvideo_name,CV_FOURCC('M', 'P', '4', '2'),30,cvSize(p.w,p.h),1);
                ...
                for(y = 0; y < p.h; ++y){
                    for(x = 0; x < p.w; ++x){
                        for(k= 0; k < p.c; ++k){
                            disp->imageData[y*step + x*p.c + k] = (unsigned char)(copy.data[k*copy.h*copy.w + y*copy.w + x]*255);
                        }
                    }
                }
        ...
        //pthread_join(fetch_thread, 0);
        //pthread_join(detect_thread, 0);
        //printf("read from outside join : %s\n",tag_results);
        //++count;
        if(count % 10 == 0){
            update_progress((int)(count * 100.0 / total_videoframes));
            fflush(outputtext);
        }
    }
...
void demo_compare(char *cfg1, char *weight1, char *cfg2, char *weight2, float thresh, int cam_index, const char *filename, char **names, int classes, int delay, char *prefix, int avg_frames, float hier, int w, int h, int frames, int fullscreen)
{
    ...
    while(!demo_done){
        buff_index = (buff_index + 1) %3;
        if(pthread_create(&fetch_thread, 0, fetch_in_thread, 0)) error("Thread creation failed");
        if(pthread_create(&detect_thread, 0, detect_in_thread, tag_results)) error("Thread creation failed");
        if(!prefix){
            fps = 1./(what_time_is_it_now() - demo_time);
            demo_time = what_time_is_it_now();
            display_in_thread(0);
